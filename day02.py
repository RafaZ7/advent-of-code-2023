...
class Game:
    ...
    def is_valid(self, cube_set: CubeSet) -> bool:
        return all(
            [game_cube_set.is_valid(cube_set) for game_cube_set in self.cube_set]
        )
    ...
    def from_string(cls, string: str) -> Self:
        ...
        cube_set = [
            CubeSet.from_string(cube_set) for cube_set in cube_set_string.split(";")
        ]
...
def game_solver_1(games_string: str, max_set: CubeSet) -> int:
    games = [
        ...
        for game_string in games_string.strip().split("\n")
    ]
