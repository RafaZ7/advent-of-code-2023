...
class CubeSet:
    ...
    def is_valid(self, other: 'CubeSet') -> bool:
        diff_set = other - self
        return diff_set.blue >= 0 and diff_set.green >= 0 and diff_set.red >= 0
...
class Game:
    ...
    def is_valid(self, cube_set: CubeSet) -> bool:
        for game_cube_set in self.cube_set:
            if not game_cube_set.is_valid(cube_set):
                return False
        return True
