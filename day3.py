...
if __name__ == "__main__":
    ...
    for i, row in enumerate(splitted_input):
        numbers_row = [Number(int(m.group()), i, m.start(), m.end() - 1) for m in re.finditer(REGEX_DIGITS, row)]
        special_characters_row = [
            SpecialCharacter(m.group(), i, m.start()) for m in re.finditer(REGEX_SPECIAL_CHARACTER, row)
        ]
        ...
        for special_character in special_characters_row:
            for row_number in [row - 1, row, row + 1]:
                ...
                for number in numbers[row_number]:
                    if number.is_adjacent_to_character(special_character) and number not in final_list_of_numbers:
    ...
    part_2_result = sum([np.prod([g.number for g in gear]) for gear in gear_list if len(gear) == 2])
